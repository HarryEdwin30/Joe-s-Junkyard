...
if weapon_type = 2 and sho_can_shoot = true{
    ...
    while (pellets_shot < max_pellets) {
        pellets_shot += 1;
        var pellet_x = irandom_range(mouse_x - zone_size, mouse_x + zone_size);
        var pellet_y = irandom_range(mouse_y - zone_size, mouse_y + zone_size);
        array_push(pellet_points, [pellet_x, pellet_y]);
    	if !collision_line(gun_start_x, gun_start_y, pellet_x, pellet_y,  tilemap, true, undefined){
            var zombies_in_sights = ds_list_create();
            var target = obj_zombie_parent;
            if collision_line_list(gun_start_x, gun_start_y, pellet_x, pellet_y, target, true, true, zombies_in_sights, true){
                var zombie_to_shoot = ds_list_find_value(zombies_in_sights, 0);
                deal_damage(rev_damage, sound_to_play, zombie_to_shoot);
            }
            if position_meeting(mouse_x, mouse_y, obj_player){
